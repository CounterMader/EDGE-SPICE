...
void c_ac_g1_stamp(CKTcircuit *circuit, ELM_TAB *element){
    if(element -> node1 -> number != 0){
        circuit -> MNAmat_comp -> data[element -> node1 -> number - 1][element -> node1 -> number - 1].im = circuit -> omega * element -> value;
    }
    if(element -> node2 -> number != 0){
        circuit -> MNAmat_comp -> data[element -> node2 -> number - 1][element -> node2 -> number - 1].im = circuit -> omega * element -> value;
    }
    if(element -> node1 -> number != 0 && element -> node2 -> number != 0){
        circuit -> MNAmat_comp -> data[element -> node1 -> number - 1][element -> node2 -> number - 1].im = -circuit -> omega * element -> value;
        circuit -> MNAmat_comp -> data[element -> node2 -> number - 1][element -> node1 -> number - 1].im = -circuit -> omega * element -> value;
    }
}
...
void l_ac_g2_stamp(CKTcircuit *circuit, ELM_TAB *element){
    element -> index_in_RHS = get_RHS_index(circuit);
    //MNA stamp
    circuit -> MNAmat_comp -> data[element -> index_in_RHS - 1][element -> index_in_RHS - 1].im = -circuit -> omega * element -> value;
    if(element -> node1 -> number != 0){
        circuit -> MNAmat_comp -> data[element -> index_in_RHS - 1][element -> node1 -> number - 1].re = 1;
        circuit -> MNAmat_comp -> data[element -> node1 -> number - 1][element -> index_in_RHS - 1].re = 1;
    }
...
void v_ac_stamp(CKTcircuit *circuit, SRC_TAB *source){
    ...
    circuit -> RHSmat_comp -> data[source -> index_in_RHS - 1][0].re = source -> src_coefficient[ac_amp] * cos(source -> src_coefficient[ac_phi]*PI/180);
    circuit -> RHSmat_comp -> data[source -> index_in_RHS - 1][0].im = source -> src_coefficient[ac_amp] * sin(source -> src_coefficient[ac_phi]*PI/180);
    if(source -> node1 -> number != 0){
...
void i_ac_g1_stamp(CKTcircuit *circuit, SRC_TAB *source){
    if(source -> node1 -> number != 0){
        circuit -> RHSmat_comp -> data[source -> node1 -> number - 1][0].re = -source -> src_coefficient[ac_amp] * cos(source -> src_coefficient[ac_phi]*PI/180);    
        circuit -> RHSmat_comp -> data[source -> node1 -> number - 1][0].im = -source -> src_coefficient[ac_amp] * sin(source -> src_coefficient[ac_phi]*PI/180);
    }
    if(source -> node2 -> number != 0){
        circuit -> RHSmat_comp -> data[source -> node2 -> number - 1][0].re = -source -> src_coefficient[ac_amp] * cos(source -> src_coefficient[ac_phi]*PI/180);    
        circuit -> RHSmat_comp -> data[source -> node2 -> number - 1][0].im = -source -> src_coefficient[ac_amp] * sin(source -> src_coefficient[ac_phi]*PI/180);
    }
